...
const imageUrl = (v) => { if (!v) return ''; if (typeof v === 'string') { const t=v.trim(); if(!t||t==='[object Object]') return ''; if((t.startsWith('{')&&t.endsWith('}'))||(t.startsWith('[')&&t.endsWith(']'))){ try{return imageUrl(JSON.parse(t));}catch(_){} } if(t.startsWith('//'))return `https:${t}`; if(t.startsWith('http://res.cloudinary.com/'))return t.replace('http://','https://'); return t; } if (Array.isArray(v)) return imageUrl(v[0]); return imageUrl(v.secure_url || v.secureUrl || v.url || v.src || v.path || v.image || v.imageUrl); };
