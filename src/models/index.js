...
const OutletProduct = models.OutletProduct || model('OutletProduct', new Schema({
  outletId:objectId('Outlet',true), productId:objectId('Product',true), enabled:{type:Boolean,default:true,index:true}, available:{type:Boolean,default:true,index:true}, stockQuantity:{type:Number,default:0,min:0}, reservedQuantity:{type:Number,default:0,min:0}, lowStockThreshold:{type:Number,default:3,min:0}, priceOverride:{type:Number,min:0}, offerPriceOverride:{type:Number,min:0}, costPrice:{type:Number,min:0}, preparationMinutes:{type:Number,default:20,min:0}, version:{type:Number,default:0}, stockInitialized:{type:Boolean,default:false,index:true}, lastStockUpdatedAt:Date, lastStockUpdatedBy:objectId('User')
},{timestamps:true}));
